...
from models.encoder import TSIEncoder, TSIEncoderTransformerTrend
from utils import take_per_row, split_with_nan, centerize_vary_length_series, torch_pad_nan
from models.trasnformer_encoder import TransformerEncoder
...
class TSI:
    def __init__(self,
        ...
        # Usando uma abordagem híbrida onde aplico uma trnasformer sobre a tendência 
        # TSIEncoderTransformerTrend para aplicar uma camada Transformer sobre a tendência
        
        self.net = TSIEncoderTransformerTrend(
            input_dims=input_dims, output_dims=output_dims,
            kernels=kernels,
            length=max_train_length,
            hidden_dims=hidden_dims, depth=depth,
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False):
        ...
        optimizer = torch.optim.SGD([p for p in self.tsi.parameters() if p.requires_grad],
                                    lr=self.lr,
                                    momentum=0.9,
                                    weight_decay=1e-4)
